...
model.compile(loss='mse', optimizer='adam', 
            #  metrics=['accuracy'])
             metrics=['mse'])
model.fit([[x1_train, y1_train], [x2_train, y2_train]], epochs=150, batch_size=1, validation_data=([x1_val, y1_val]))	#fit = 기계를 트레이닝 시킴, 

# 4. 평가 예측
loss, mse = model.evaluate([[x1_test, y1_test],[x2_test, y2_test]], batch_size=1) # a[0], a[1] 
print('mse: ', mse) #1.0
print('loss: ', loss)
